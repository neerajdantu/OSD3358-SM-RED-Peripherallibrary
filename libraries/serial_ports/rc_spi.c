...
#define SPI00_PATH			"/dev/spidev0.0"
#define SPI01_PATH			"/dev/spidev0.1"
#define SPI_MAX_SPEED		24000000 	// 24mhz
#define SPI_MIN_SPEED		1000		// 1khz
#define SPI_BITS_PER_WORD 	8
#define SPI_BUF_SIZE		2

int fd[4];			// file descriptor for SPI1_PATH device cs0, cs1
int initialized[4];	// set to 1 after successful initialization 
...
int rc_spi_init(ss_mode_t ss_mode, int spi_mode, int speed_hz, int slave){
	...
	case 2:
		...
		break;
	case 3:
		fd[2] = open(SPI00_PATH, O_RDWR);
		if(fd[2] < 0) {
			printf("ERROR: %s missing\n", SPI00_PATH);
		}
	case 4:
		fd[3] = open(SPI01_PATH, O_RDWR);
		if(fd[3] < 0) {
			printf("ERROR: %s missing\n", SPI01_PATH);
		}
	default:
		printf("ERROR: SPI slave must be 1, 2, 3 or 4\n");
		return -1;
	}
	// set settings
	...
	// set up slave select pins ------------------------------------------------------------------commented for osd3358-SM reference design
...
int rc_spi_fd(int slave){
	...
	case 3:
		if(initialized[2] == 0){
			printf("ERROR: SPI0 slave 1 not initializes yet\n");
			return -1;
		}
		else return fd[2];
	case 4:
		if(initialized[3] == 0){
			printf("ERROR: SPI0 slave 2 not initialized yet\n");
		}
		else return fd[3];
	}
...
int rc_spi_transfer(char* tx_data, int tx_bytes, char* rx_data, int slave){
	int ret;
	// sanity checks
	if(slave!=1 && slave!=2 && slave != 3 && slave != 4){
		printf("ERROR: SPI slave must be 1, 2, 3 or 4\n");
		return -1;
	}
	if(initialized[slave-1]==0){
